...
import 'package:vcare_app/feature/search/presentation/views/widgets/doctor_list_view_item_horizontal.dart';
      ...
      if (state is SearchSuccessState||state is GetSearchedDoctorsList) {
        return ListView.separated(
          ...
          itemCount: SearchCubit.get(context).searchController.text.isEmpty
              ...
              : SearchCubit.get(context).searchedDoctorList.length,
          physics: const BouncingScrollPhysics(),
          itemBuilder: (context, index) => InkWell(
            onTap: () {
              Navigator.pushNamed(context, Routes.doctorDetailsView,
                  arguments: SearchCubit.get(context).doctorsList![index]);
            },
            child: DoctorListViewItemHorizontal(
              doctor: SearchCubit.get(context).searchController.text.isEmpty
                  ? SearchCubit.get(context).doctorsList![index]
                  : SearchCubit.get(context).searchedDoctorList[index],
            ),
          ),
